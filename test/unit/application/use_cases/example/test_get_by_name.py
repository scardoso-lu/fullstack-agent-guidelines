...
async def test_get_found_returns_dto(mock_repo):
    mock_repo.get_by_name.return_value = Example._mock("domain/01_entity", "domain", "Test entity")

    result = await GetExampleByNameUseCase(mock_repo).execute("domain/01_entity")

    assert isinstance(result, ExampleDto)
    assert result.name == "domain/01_entity"
    assert result.layer == "domain"
    assert result.description == "Test entity"
    mock_repo.get_by_name.assert_called_once_with("domain/01_entity")


@pytest.mark.asyncio
async def test_get_strips_whitespace(mock_repo):
    mock_repo.get_by_name.return_value = Example._mock("domain/01_entity", "domain")
    await GetExampleByNameUseCase(mock_repo).execute("  domain/01_entity  ")
    mock_repo.get_by_name.assert_called_once_with("domain/01_entity")
...
async def test_get_not_found_raises_not_found_error(mock_repo):
    ...
    with pytest.raises(NotFoundError):
        await GetExampleByNameUseCase(mock_repo).execute("domain/99_missing")
