...
async def test_get_all_loads_fixture_files():
    ...
    assert "backend/domain/01_test_fixture" in names
    assert "backend/application/01_test_fixture" in names
    assert "frontend/01_test_fixture" in names


@pytest.mark.asyncio
async def test_get_by_name_found_backend():
    repo = ExampleRepository(FIXTURES_DIR)
    e = await repo.get_by_name("backend/domain/01_test_fixture")
    assert e is not None
    assert e.stack == "backend"
    assert e.layer == "domain"
    assert e.filename == "01_test_fixture.py"


@pytest.mark.asyncio
async def test_get_by_name_found_frontend():
    repo = ExampleRepository(FIXTURES_DIR)
    e = await repo.get_by_name("frontend/01_test_fixture")
    assert e is not None
    assert e.stack == "frontend"
    assert e.layer == "frontend"
    assert e.filename == "01_test_fixture.ts"


@pytest.mark.asyncio
async def test_get_by_name_missing_returns_none():
    repo = ExampleRepository(FIXTURES_DIR)
    e = await repo.get_by_name("backend/domain/99_nonexistent")
    assert e is None
...
async def test_get_by_layer_returns_correct_examples():
    ...
    assert results[0].stack == "backend"


@pytest.mark.asyncio
async def test_get_by_layer_frontend_returns_frontend_examples():
    repo = ExampleRepository(FIXTURES_DIR)
    results = await repo.get_by_layer("frontend")
    assert len(results) == 1
    assert results[0].stack == "frontend"
