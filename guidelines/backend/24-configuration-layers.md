...
The settings page is an admin-only route. It fetches all `AppConfig` rows and renders a form. Each field saves on blur or on explicit submit via a Server Action.
...
|---|---|---|
| A secret (key, password, token) | 1 | `.env` / env var |
| A service URL or port | 1 | `.env` / env var |
| A build-time flag (debug mode) | 1 | `.env` / env var |
| A user-adjustable limit (upload size) | 2 | `app_config` table |
| A user-adjustable preference (language) | 2 | `app_config` table |
| A feature flag ops toggles without redeploy | 2 | `app_config` table |
...
MAX_FILE_SIZE = 10 * 1024 * 1024   # buried in a module; impossible to change

# ❌ WRONG — runtime preference in env var (requires redeploy to change)
class Settings(BaseSettings):
    DEFAULT_LANGUAGE: str = "en"

# ❌ WRONG — secret in the database (rotatable, auditable in the wrong place)
