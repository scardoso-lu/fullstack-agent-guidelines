...
_logger = get_logger("use_case.validate_commit_messages")
...
_MAX_SUBJECT_LEN = 72
# Auto-generated subjects we skip rather than flag
_SKIP_RE = re.compile(r"^(Merge |Revert \")")


class ValidateCommitMessagesUseCase:
    async def execute(self, git_log: str) -> AnalysisReportDto:
        ...
        for raw in raw_commits:
            ...
            if len(subject) > _MAX_SUBJECT_LEN:
                findings.append(FindingDto(
                    ...
                    location=commit_hash,
                    message=(
                        f"Subject is {len(subject)} chars (max {_MAX_SUBJECT_LEN}): {subject!r}"
                    ),
                    hint=(
        ...
        summary = (
            ...
            f"found {required_count} format violation(s) and "
            f"{recommended_count} recommendation(s)."
            if findings
            else f"All {checked} commit(s) follow the Conventional Commits format."
        )

        _logger.info(
            "validate_commits commits=%d findings=%d status=%r",
            checked,
            len(findings),
            status,
        )
