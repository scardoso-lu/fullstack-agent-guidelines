...
_logger = get_logger("use_case.validate_migration")
...
_LOOKAHEAD = 10  # lines to inspect after op.add_column(


class ValidateMigrationUseCase:
    async def execute(self, source: str) -> AnalysisReportDto:
        if not source.strip():
            raise ValueError(
        ...
        _logger.info(
            "validate_migration lines=%d findings=%d status=%r",
            len(lines),
            len(findings),
            status,
        )
